...
class Earley extends JPanel {
	...
	public Earley(int length, String grammerfile, String sentencefile)
			throws Exception {
		...
		if (found == 1) {
			...
			System.out
					.println("parse error:check the grammer and te input sentence");
		}
	}
	...
	void EarleyParse(String grammerfile, String sentencefile) throws Exception {
		...
		while ((tgrammer = br1.readLine()) != null) // read the grammer file
		{
			...
			if (tempgrammer1[1].indexOf('&') != -1) {
				...
				grammerhash.put(tempgrammer1[0], grammerrule); // extract the
																// grammer
			}// endif
			else {
		...
		while ((tgrammer1 = br2.readLine()) != null) // read the file
		{
			...
			for (int u = 1; u < tempgrammer3.length; u++)
				if (nonterminals.contains(tempgrammer3[u])) {
					...
					if ((tempgrammer3[u].equals("->"))
							| (tempgrammer3[u].equals("&"))) {
					}// endif
					else {
		...
		for (int i = 0; i < nonterminals.size(); i++) // construct part of
														// speech
		{
			int flag = 0;
		...
		for (int x = 0; x < (length + 1); x++) // implement the parser
		{
			for (int z = 0; z < charts[x].table.size(); z++) {
				...
				if (temprule.dot != temprule.rule.size()) {
					if ((nonterminals.indexOf((String) temprule.rule
							.elementAt(temprule.dot))) != -1) {
		...
		for (int y = 0; y < charts[charts.length - 1].table.size(); y++) {
			rules temprules = (rules) (charts[charts.length - 1].table
					.elementAt(y));
			if ((((String) temprules.rule.elementAt(0)).equals("S"))
					& (temprules.dot == temprules.rule.size())) {
	...
	public void enqueue(rules r, int i, int check) // insert rule in the chart
	{
		...
		for (int t = 0; t < charts[i].table.size(); t++) // check whether the
															// rule already
															// exists in the
															// chart
		{
				...
				if (check == 2) {
					if (temp.pointer.size() == r.pointer.size()) {
						...
						for (int k = 0; k < temp.pointer.size(); k++) {
							if (((Integer) temp.pointer.elementAt(k))
									.intValue() != ((Integer) r.pointer
	...
	{
		Vector v2 = (Vector) grammerhash.get(temprule.rule
				.elementAt(temprule.dot));
		for (int i = 0; i < (v2.size()); i++) {
			rules temprule1 = new rules(1, temprule.end, temprule.end,
					((String) temprule.rule.elementAt(temprule.dot)) + " "
							+ ((String) v2.elementAt(i)), 'P');
	...
	public void scanner(rules temprule) // the scanner
	{
		for (int i = 0; i < lexicon[temprule.end].table.size(); i++) {
			if (((String) temprule.rule.elementAt(temprule.dot))
					.equals((String) lexicon[temprule.end].table.elementAt(i))) {
				// System.out.println("ttttttttttttttttttttttt"+temprule.end+lexicon[temprule.end].table.size());
				enqueue(new rules(2, temprule.end, temprule.end + 1,
						((String) lexicon[temprule.end].table.elementAt(i))
								+ " " + sentencefile1[temprule.end], 'S'),
						temprule.end + 1, 1);
			}// endif
		}
	}// end scanner
	...
	public void completor(rules temprule) // the completor
	{
		for (int t = 0; t < charts[temprule.start].table.size(); t++) {
			...
			if (temprule1.rule.size() != temprule1.dot)
				if (((String) temprule.rule.elementAt(0))
						.equals((String) temprule1.rule
								.elementAt(temprule1.dot))) {
					Vector v2 = (Vector) temprule1.pointer.clone();
					v2.addElement(new Integer(temprule.rno));
					enqueue(new rules(temprule1.dot + 1, temprule1.start,
							temprule.end, temprule1.rule, v2, 'C'),
							temprule.end, 2);
				}// endif
		}// endfor
	}// end completor
	...
	public void displaychart() throws Exception {
		...
		for (int x = 0; x < chartlength; x++) {
			...
			for (int y = 0; y < charts[x].table.size(); y++) {
				...
				str = str + " [" + trules.start + "," + trules.end + "]"
						+ "   " + trules.rno + "    ";
			...
			for (int z = 0; z < charts[x].table.size(); z++) {
				...
				if (temprule.rno == rlno) {
					...
					for (int y = temprule.pointer.size() - 1; y >= 0; y--) {
						flag = 1;
						n.pointers.addElement(tree(((Integer) temprule.pointer
								.elementAt(y)).intValue()));
					}// endfor
					if (flag == 0) {

						n.pointers.addElement(new node((String) temprule.rule
								.elementAt(1)));
					}// endif
					return n;
				}// endif
